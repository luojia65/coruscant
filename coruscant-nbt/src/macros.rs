macro_rules! return_expr_for_serialized_types_method {
    ...
    ($expr:expr, $func:ident($($arg:ty),*), result: $result:path) => {
        #[inline]
        fn $func(self, $(_: $arg,)*)
                 -> ::core::result::Result<$result, Self::Error>
        {
            $expr
        }
    };
...
macro_rules! return_expr_for_serialized_types_helper {
    ...
    ($expr:expr, i128) => {
        return_expr_for_serialized_types_method!{$expr, serialize_i128(i128)}
    };
    ($expr:expr, u8) => {
    ...
    ($expr:expr, u128) => {
        return_expr_for_serialized_types_method!{$expr, serialize_u128(u128)}
    };
    ($expr:expr, f32) => {
